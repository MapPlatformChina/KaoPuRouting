...
class RFBaseChart:
	...
	def set_data(self, value_array):
		...
		if len(self.value_array) > 0:
			...
			self.max_value = 100
		self.dynamic_image_obj = Image.new('RGBA', (self.width, self.height))
		draw_obj = ImageDraw.Draw(self.dynamic_image_obj)
		self.draw_axis_y_value(draw_obj)
		self.draw_value(draw_obj)
		self.dynamic_photo = ImageTk.PhotoImage(self.dynamic_image_obj)
	...
	def draw_axis_y_value(self,draw_obj):
...
def main():
	...
	basechart_obj_a = RFBaseChart(350,250,False)
	basechart_obj_b = RFBaseChart(350,250,True)
	
	photo_static_a = basechart_obj_a.get_static_photo()
	map_canvas.create_image(1400,250, image= photo_static_a)
	photo_static_b = basechart_obj_b.get_static_photo()
	map_canvas.create_image(1400,480, image= photo_static_b)	
	
	value_array = []
	for i in range(0,288):
		value_array.append(i)
	basechart_obj_a.set_data(value_array)
	basechart_obj_b.set_data(value_array)
	
	photo_dynamic_a = basechart_obj_a.get_dynamic_photo()
	map_canvas.create_image(1400,250, image= photo_dynamic_a)
	photo_dynamic_b = basechart_obj_b.get_dynamic_photo()
	map_canvas.create_image(1400,480, image= photo_dynamic_b)	
